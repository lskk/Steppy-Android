...
public class TBGameMeasurement {
	
	public static final String TAG					= "Table Game Measurement";
	
	public static final String TABLE_GAME_MEASUREMENT 	= "measurement_game";
	...
	public static final String INITIAL_CREATE = "create table "
			+ TABLE_GAME_MEASUREMENT + "(" + COL_ID + " integer primary key autoincrement, "
	...
	public TBGameMeasurement(Context context) {
	...
	public long insert(ContentValues values) {
		...
		Long result = db.insert(TABLE_GAME_MEASUREMENT, null, values);
		
		if(result != -1) {
			Log.i(TAG, "inserting value "+ values.getAsString(TBGameMeasurement.COL_VALUE) +" to table measurement succeed");
			lastStep = values.getAsString(TBGameMeasurement.COL_VALUE);
		} else {
	...
	public int update(String tID, ContentValues values) {
		...
		int result = db.update(TABLE_GAME_MEASUREMENT, values, COL_ID + "=?", new String[] {tID});
	...
	public int updateAfterSync(ArrayList<String> pID) {
		...
		int result = db.update(TABLE_GAME_MEASUREMENT, values, COL_IS_SYNC + "=? and "
	...
	public Cursor getAllData() {
		...
		Cursor c = db.query(TABLE_GAME_MEASUREMENT, projection, null, null, null, null, null);
	...
	public Cursor getLastInsert() {
		...
		Cursor c = db.query(TABLE_GAME_MEASUREMENT, projection, null, null, null, null, COL_DATE + " DESC");
	...
	public Cursor getDataToSync() {
		...
		Cursor c = db.query(TABLE_GAME_MEASUREMENT, projection, COL_IS_SYNC + "=?", new String[] {"0"}, null, null, null, "100");
	...
	public Cursor getLastWeekStep() {
		...
		Cursor c = db.query(TABLE_GAME_MEASUREMENT, projection, COL_DATE + " >= ? and " 
	...
	public void delete() {
		...
		int result = db.delete(TABLE_GAME_MEASUREMENT, null, null);
		if (result > 0) {
